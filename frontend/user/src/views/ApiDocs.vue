<template>
  ...
  -d '{
    ...
    <el-card v-if="channelGroups.length > 1" shadow="hover" class="doc-card">
      ...
      <pre class="code-block">claude-sonnet-5         {{ t('apiDocs.groupExampleEcon') }}
claude-fable-5     {{ t('apiDocs.groupExamplePro') }}</pre>
...
cc</pre>
          ...
          <ul class="config-list">
            ...
            <li><code>/model claude-sonnet-5</code> — {{ t('apiDocs.ccFull5Cmd3').split('—')[1] }}</li>
...
cc  # 走官方</pre>
        ...
        <!-- Codex App (TransitAI 推荐 ⭐) -->
        <el-tab-pane label="Codex App ⭐" name="codex">
          <h4 class="tut-title">通过 cc-switch 接入 Codex App</h4>
          <p class="desc-text">TransitAI 完整兼容 OpenAI Responses API，支持 25+ 模型 (GPT/Claude/Gemini/DeepSeek/MiniMax/Qwen)。推荐通过 cc-switch 桌面版一键接入。</p>

          <div class="tut-step">步骤 1：下载安装 cc-switch</div>
          <p class="desc-text">访问 <a href="https://ccswitch.io" target="_blank" rel="noopener">ccswitch.io</a> 下载对应平台版本（Windows / macOS / Linux），安装后启动应用。</p>

          <div class="tut-step">步骤 2：添加 TransitAI Provider</div>
          <p class="desc-text">在 cc-switch 中点击 "添加 Provider"，类型选择 "自定义"，填入下列配置：</p>
          <pre class="code-block">名称（Name）: TransitAI
Base URL: https://transitai.cloud/v1
API Key: 你的 TransitAI sk-xxx
模型（Model）: gpt-5.4
Wire API: responses</pre>

          <div class="tut-step">步骤 3：启用 TransitAI 并启动 Codex App</div>
          <p class="desc-text">cc-switch 会自动生成 Codex 配置文件。点击 "Use" 启用 TransitAI，然后启动或重启 Codex App 即可。</p>

          <div class="tut-step">手动配置（不使用 cc-switch）</div>
          <p class="desc-text">编辑 <code>~/.codex/config.toml</code>（Windows: <code>%USERPROFILE%\.codex\config.toml</code>）：</p>
          <pre class="code-block">model_provider = "transitai"
model = "gpt-5.4"

[model_providers.transitai]
name = "TransitAI"
...
requires_openai_auth = false</pre>
          ...
          <p class="desc-text">编辑 <code>~/.codex/auth.json</code>：</p>
          <pre class="code-block">{
  "OPENAI_API_KEY": "你的 TransitAI sk-xxx"
}</pre>
...
msg = client.messages.create(
    model="claude-fable-5",   # -pro 后缀 → 官方直连
    max_tokens=1024,
    messages=[
        {"role": "user", "content": "Hello"}
    ]
