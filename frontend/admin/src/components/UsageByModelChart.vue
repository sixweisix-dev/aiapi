<template>
  <div class="data-card">
    ...
    <div v-else-if="!hasData" class="empty-tip">最近 30 天暂无消耗数据</div>
    <v-chart v-else :option="chartOption" autoresize style="height: 380px" />
  </div>
</template>
...
async function load() {
  try {
    const res = await api.get('/admin/usage/by-model?days=30')
