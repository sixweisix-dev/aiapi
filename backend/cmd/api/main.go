...
responsesHandler := handlers.NewResponsesHandler(db, pool, billingEngine, channelTracker, redisClient)
	...
	oauthHandler := handlers.NewOAuthHandler(db, redisClient, cfg.JWTSecret)
...
stripeHandler := handlers.NewStripeHandler(db, billingEngine)
	...
	auth := r.Group("/v1/auth")
	{
		...
		auth.GET("/github/start", oauthHandler.GithubStart)
		auth.GET("/github/callback", oauthHandler.GithubCallback)
		auth.GET("/google/start", oauthHandler.GoogleStart)
...
v1.DELETE("/responses/:id", responsesHandler.Delete)
	...
	r.POST("/v1/internal/zhifux-cleanup", cronHandler.ZhifuxCleanup)
...
// 支付FM (zhifux)
r.GET("/v1/zhifux/webhook", zhifuxHandler.Webhook)
...
r.GET("/v1/user/zhifux/order/:order_no", middleware.JWTAuth(cfg.JWTSecret), zhifuxHandler.QueryOrder)
	...
	admin.Use(middleware.AdminRequired())
	{
		...
		admin.GET("/access-logs", handlers.ListAccessLogs)
